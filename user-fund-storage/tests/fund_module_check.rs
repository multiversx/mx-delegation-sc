...
pub fn check_consistency_for_type(
    ...
    while id > 0 {
        ...
        // check next/prev
        assert_eq!(fund_item.type_list_prev, prev_id, "type list inconsistency: bad prev");
    ...
    assert_eq!(sum, type_list.total_balance, "type list inconsistency: bad sum");
}
...
pub fn check_consistency_for_user_type(
    ...
    while id > 0 {
        ...
        if fund_item.user_list_next == 0 {
            assert_eq!(user_type_list.last, id, "user-type list inconsistency: bad list last");
        }
    ...
    assert_eq!(sum, user_type_list.total_balance, "user-type list inconsistency: bad sum");
}
