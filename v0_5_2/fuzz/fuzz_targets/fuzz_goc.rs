...
use delegation_v0_5_2::reset_checkpoint_types::*;
use elrond_wasm_debug::RustBigUint;

fuzz_target!(|data: &[u8]| {
    if let Ok(decoded) = GlobalOperationCheckpoint::<RustBigUint>::top_decode(data) {
        let encoded_clean = check_top_encode(&decoded);
        let decoded_again = check_top_decode::<GlobalOperationCheckpoint<RustBigUint>>(&encoded_clean[..]);
